...
![](https://tva1.sinaimg.cn/large/007S8ZIlgy1giyev3qmsaj30hm07pt9l.jpg)

![](https://user-gold-cdn.xitu.io/2020/3/1/1709652a7948d1b8?imageslim)
...
|        | 和源数据是否指向同一对象 | 第一层数据为基本数据类型 | 源数据中包含子对象       |
| ------ | ------------------------ | ------------------------ | ------------------------ |
| 赋值   | 是                       | 改变会使源数据一同改变   | 改变会使源数据一同改变   |
| 浅拷贝 | 否                       | 改变不会使源数据一同改变 | 改变会使源数据一同改变   |
| 深拷贝 | 否                       | 改变不会使源数据一同改变 | 改变不会使源数据一同改变 |
...
![](https://user-gold-cdn.xitu.io/2018/7/30/164e6d5f642d440a?imageslim)

这也是利用 JSON.stringify 将对象转成 JSON 字符串，再用 JSON.parse 把字符串解析成对象，一去一来，新的对象产生了，而且对象会开辟新的栈，实现深拷贝。

**这种方法虽然可以实现数组或对象深拷贝,但不能处理函数和正则**，因为这两者基于 JSON.stringify 和JSON.parse 处理后，得到的正则就不再是正则（变为空对象），得到的函数就不再是函数（变为null）了。
