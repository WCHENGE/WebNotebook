...
* [localStorage](https://www.runoob.com/jsref/prop-win-localstorage.html) 用于长久保存整个网站的数据，保存的数据没有过期时间，直到手动去除。
* [sessionStorage](https://www.runoob.com/jsref/prop-win-sessionstorage.html) 用于临时保存同一窗口(或标签页)的数据，在关闭窗口或标签页之后将会删除这些数据。
