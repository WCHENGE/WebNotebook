...
>   对象调用`toString()`变为字符串
>
> * null == undefined 相等，但是和其它值比较就不在相等了
>
...
```javascript
function myInstanceof(parse, target) {
  let proto = Object.getPropertyOf(parse)
  while(true) {
    if (proto === null) return false
    // 在当前实例对象的原型链上，找到了当前类
    if (proto === Object.getPropertyOf(target)) return true
