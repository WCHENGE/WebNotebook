...
> IFC | 行内格式化上下文（Inline Formatting Context），IFC 的 line box（线框）高度由其包含行内元素中最高的实际高度计算而来（不受到竖直方法的 padding / margin 影响）。
...
由于 `http` 存在一个明显的弊端（消息只能由客户端推送到服务器端，而服务器端不能主动推送到客户端），导致如果服务器如果有连续的变化，这时只能使用轮询，而轮询效率过低，并不适合。于是 `WebSocket`被发明出来。
...
* 原因 - 避免`DOM`渲染的冲突（JS 和 DOM 渲染共用一个线程，JS 可以修改 DOM）
...
setTimeout(function() {
  console.log('setTimeout');	// 6. "setTimeout"
}, 0);
...
console.log('script end');	// 3. "script end"
...
* Renderer 进程开始解析 css rule tree 和 dom tree，这两个过程是并行的，所以一般我会把link标签放在页面顶部。
* 解析绘制过程中，当浏览器遇到link标签或者script、img等标签，浏览器会去下载这些内容，遇到时候缓存的使用缓存，不适用缓存的重新下载资源。
* css rule tree和dom tree生成完了之后，开始合成render tree，这个时候浏览器会进行 layout，开始计算每一个节点的位置，然后进行绘制。
...
* 原始数据类型（`Undefined`、`Null`、`String`，`Number`、`Boolean`）-- 栈
* 引用数据类型（对象、数组和函数）-- 堆
* 两种类型的区别是：存储位置不同：
...
console.log(location.hash)	// 返回一个URL的锚点部分
```
...
![image-20210221181745777](https://tva1.sinaimg.cn/large/008eGmZEgy1gnvbpgovjlj31bk0hott2.jpg)
...
* session 和 cookie 的区别
  ...
  > 服务器设置 session， 服务器返回给客户端的信息，在响应头中带着 set-cookie = 'connect.$id' ，客户端会把信息种植到本地的 cookie 中 http-only ；当客户端再次向服务器发送请求的时候，会默认在请求头中将 cookies 把` connect.$id`传递发给服务器。

  1. 由于 HTTP 协议是无状态的协议，所以服务端需要记录用户的状态时，就需要用某种机制来识具体的用户，这个机制就是session。典型的场景比如购物车，当你点击下单按钮时，由于 HTTP 协议无状态，所以并不知道是哪个用户操作的，所以服务端要为特定的用户创建了特定的 session，用于标识这个用户，并且跟踪用户，这样才知道购物车里面有几本书。这个 session 是保存在服务端的，有一个唯一标识。

     > 在服务端保存 session 的方法很多，内存、数据库、文件都有。集群的时候也要考虑 session 的转移，在大型的网站，一般会有专门的 session 服务器集群，用来保存用户会话，这个时候 session 信息都是放在内存的，使用一些缓存服务比如 memcached之类的来放 session。
...
/* 转换为字符串 */
arr = arr.toString().split(',').map(item => parseInt(item, 0))
arr = JSON.stringify(arr).replace(/(\[|\])/g, '').split(',').map(item => parseInt(item, 0))
...
![image-20210221184342297](https://tva1.sinaimg.cn/large/008eGmZEgy1gnvcgd9bw7j30iw05a75t.jpg)
...
### 37. 显示类型转换（parseInt、Number）

* `parseInt` 处理的值是*字符串*，从字符串的左侧开始查找有效数字字符（遇到非有效数字字符则停止查找）；如果处理的值不是字符串，需要先转换为字符串然后再开始查找，如果还未找到有效数字字符则直接返回 NaN。
...
isNaN(Number(!!Number(parseInt("0.8"))));	// => isNaN(Number(false)) => isNaN(0) => false
typeof !parseInt(null) + !isNaN(null);	// => typeof !NaN + !isNaN(0) => 'boolean' + true => 'booleantrue'
```
...
创建值：
  ...
  + 存储键值对
    ...
    fn：自执行函数，需要把 obj.name 的值当做实参传递进来 => undefined.name

+ 结果：Uncaught TypeError：Cannot read property 'name' of undefined

![image-20210302125227252](https://tva1.sinaimg.cn/large/e6c9d24egy1go5gvo3hvxj21fi0pqaun.jpg)
...
function insert(ary) {
  ...
  for(let i=1; i<ary.length; i++) {
    for(let j=result.length-1; j>=0; j--) {
      if (ary[i] > result[j]) {
        result.splice(j+1, 0, ary[i])
        break;
      }
      if (j===0) result.unshift(ary[i])
    }
  }
  return result
}
...
function quickSort(arr) {
  let len = arr.length
  if (len === 0) return []
  const c = arr.splice(Math.floor(len / 2), 1)
  const l = []
  const r = []
  len = arr.length
  for (let i = 0; i < len; i++) {
    arr[i] < c[0] ? l.push(arr[i]) : r.push(arr[i])
  }
...
function fb(n) {
    ...
    return count == 0 ? curr : fn(--count, next, curr + next)
  }
  return fn(n)
}
```

### 5. 输入一个正数 N，输出所有和为 N 的连续正数序列
...
```javascript
function cyFn(num, middle, curr) {
  const arr = [curr]
  for(let j = curr+1; j <= middle; j++) {
    const accumulated = arr.concat(j).reduce((acc, curr) => acc + curr)
    if (accumulated<=num) arr.push(j)
    else break
  }
  return arr
}
...
function fn(num) {
  ...
  for(let i=1; i<middle; i++) {
    const cyArr = cyFn(num, middle, i)
    const accumulated = cyArr.reduce((acc, curr) => acc + curr)
    if(accumulated===num) result.push(cyArr)
  }
  return result
}
```
